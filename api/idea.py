class IDEA:
    ...
    def encrypt_string(self, plain):
        encrypted_blocks = []
        while plain:
            # Беремо 8 байтів (64 біти) для шифрування
            block = plain & ((1 << 64) - 1)
        ...
        for block in encrypted_blocks:
            encrypted_result = (encrypted_result << 64) | block
        return encrypted_result
        # Дешифрування рядка будь-якої довжини

    def decrypt_string(self, encrypted):
        decrypted_text = ""
        encrypted_blocks = []

        # Split encrypted input into 64-bit blocks
        while encrypted:
            block = encrypted & ((1 << 64) - 1)
            encrypted_blocks.append(block)
            encrypted >>= 64

        # Decrypt each block and accumulate the result
        encrypted_blocks.reverse()
        for block in encrypted_blocks:
            decrypted_block = self.decrypt(block)
            # Convert to bytes, decode, and remove leading/trailing padding
            decrypted_text += decrypted_block.to_bytes(8, 'big').decode('utf-8', errors='ignore').lstrip('\x00')

        # Ensure the entire decrypted string has no leading zeroes
        return decrypted_text.lstrip('\x00')
